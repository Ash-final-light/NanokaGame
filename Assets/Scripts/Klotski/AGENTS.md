...
- [ ] `nanoka_left_arm` 的 Sprite 左上角与棋盘左上角完全一致；
- [ ] 棋盘由该锚点计算为 `4×5`、`CellSize=1.68`；
- [ ] Board Center 计算结果为 `(0.06,0.12)`；
- [ ] 10 个棋子的逻辑格与当前场景映射一致；
- [ ] 初始空格为 `(1,0)` 和 `(2,0)`；
- [ ] 修改锚点或整体棋盘位置后无需逐个改棋子 Transform；
- [ ] 整体 Sprite Bounds 为 `6.72×8.40`。

### 22.2 输入与拖动

- [ ] 鼠标可以拖动棋子；
- [ ] 单指触摸可以拖动棋子；
- [ ] Pointer Down 不会导致棋子跳到指针中心；
- [ ] 超过阈值后正确锁定水平或垂直轴；
- [ ] 锁轴后不会中途切换为另一个轴；
- [ ] 跟手预览不会穿过其他棋子；
- [ ] 跟手预览不会超出棋盘；
...
- [ ] 所有代码位于 `NanokaGame.Games.Klotski...` 命名空间；
- [ ] Model 不依赖 Unity 表现组件；
- [ ] 坐标公式只存在于 `KlotskiBoardLayout`；
- [ ] 所有棋子没有 Missing Script；
- [ ] Unity 编译无新增 Error；
- [ ] EditMode 布局和移动测试通过；
- [ ] PlayMode 拖动和胜利流程测试通过；
- [ ] Scene 与 Prefab 没有无关序列化变化。
